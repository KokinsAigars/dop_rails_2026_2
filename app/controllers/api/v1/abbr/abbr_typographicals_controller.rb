...
class Api::V1::Abbr::AbbrTypographicalsController < Api::V1::BaseController
  MODEL = Sc01Abbreviations::AbbrTypographical

  # Policy: admin for all except show
  before_action -> { doorkeeper_authorize! :admin }, except: %i[show]

  # GET /api/v1/abbr/abbr_typographicals
  def index
    ...
    if params[:q].present?
      q = "%#{params[:q]}%"
      scope = scope.where(
        "abbr_name ILIKE ? OR abbr_id_est ILIKE ?",
        q, q
      )
    end
  ...
  # GET /api/v1/abbr/abbr_typographicals/:id  (public)
  def show
  ...
  # POST /api/v1/abbr/abbr_typographicals
  def create
  ...
  # PATCH /api/v1/abbr/abbr_typographicals/:id  (new version)
  def update
    set_db_audit_reason("abbr_typographicals api update -> new version") if respond_to?(:set_db_audit_reason, true)
  ...
  # GET /api/v1/abbr/abbr_typographicals/:id/history
  def history
    row = MODEL.find(params[:id])
