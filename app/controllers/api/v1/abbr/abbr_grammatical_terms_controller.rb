...
class Api::V1::Abbr::AbbrGrammaticalTermsController < Api::V1::BaseController
  MODEL = Sc01Abbreviations::AbbrGrammaticalTerm

  # Policy: admin for all except show
  before_action -> { doorkeeper_authorize! :admin }, except: %i[show]

  # GET /api/v1/abbr/abbr_grammatical_terms
  def index
    ...
    if params[:q].present?
      q = "%#{params[:q]}%"
      scope = scope.where(
        "abbr_name ILIKE ? OR abbr_lv ILIKE ? OR abbr_id_est ILIKE ?",
        q, q, q
      )
    end
  ...
  # GET /api/v1/abbr/abbr_grammatical_terms/:id  (public)
  def show
  ...
  # POST /api/v1/abbr/abbr_grammatical_terms
  def create
  ...
  # PATCH /api/v1/abbr/abbr_grammatical_terms/:id  (new version)
  def update
    set_db_audit_reason("abbr_grammatical_terms api update -> new version") if respond_to?(:set_db_audit_reason, true)
  ...
  # GET /api/v1/abbr/abbr_grammatical_terms/:id/history
  def history
    row = MODEL.find(params[:id])
