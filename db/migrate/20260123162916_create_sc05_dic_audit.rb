...
class CreateSc05DicAudit < ActiveRecord::Migration[8.1]
  def up
    ...
    create_table "sc_05_audit.abbr_audit_events", id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    create_table "sc_05_audit.ref_audit_events", id: :uuid, default: -> { "gen_random_uuid()" } do |t|
      t.timestamp :event_at, null: false, default: -> { "CURRENT_TIMESTAMP" }
      t.text      :action, null: false
      t.text      :table_name, null: false
      t.uuid      :row_id, null: false
    ...
    create_table "sc_05_audit.dic_entry_audit_events", id: :uuid, default: -> { "gen_random_uuid()" } do |t|
      t.timestamp :event_at, null: false, default: -> { "CURRENT_TIMESTAMP" }
      t.text      :action, null: false
      t.text      :table_name, null: false
      t.uuid      :row_id, null: false
